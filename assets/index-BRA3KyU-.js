...
<g class="tara pose-idle" data-mood="neutral">
  ...
  <!-- neck -->
  <path class="skin neck" d="M -10 ${$+6} L 15 ${$+6}
    L 13 ${Le-6} L -8 ${Le-6} Z" />
  ...
  <g class="body">
    <path class="kurta" d="M -26 ${$-2}
      C -38 ${$+26} -40 ${y-40} -42 ${y-6}
      L 42 ${y-6}
      C 40 ${y-40} 38 ${$+26} 26 ${$-2}
      C 14 ${$-14} -14 ${$-14} -26 ${$-2} Z" />

    <!-- the V of the neckline, with a gold chain sitting in it -->
    <path class="kurta-vee" d="M -13 ${$-8}
      C -7 ${$+16} 9 ${$+16} 15 ${$-8}" />
    <path class="necklace" d="M -16 ${$+2} C -8 ${$+24} 10 ${$+24} 18 ${$+2}" />
    <circle class="pendant" cx="1" cy="${$+26}" r="4.2" />

    <!-- gold waist band -->
    <path class="sash" d="M -42 ${y-8} L 42 ${y-8} L 42 ${y+8} L -42 ${y+8} Z" />

    <!-- the skirt, flared, with the gold hem the reference gives it -->
    <path class="skirt" d="M -42 ${y+6}
      C -50 ${y+26} -55 ${y+44} -57 ${y+58}
      C -28 ${y+70} 30 ${y+70} 57 ${y+58}
      C 55 ${y+44} 50 ${y+26} 42 ${y+6} Z" />
    <path class="skirt-pleat" d="M -20 ${y+12} L -25 ${y+60}" />
    <path class="skirt-pleat" d="M 2 ${y+12} L 2 ${y+64}" />
    <path class="skirt-pleat" d="M 24 ${y+12} L 29 ${y+60}" />
    <path class="kurta-hem" d="M -57 ${y+58}
      C -28 ${y+70} 30 ${y+70} 57 ${y+58}
      C 57 ${y+72} 57 ${y+72} 57 ${y+72}
      C 30 ${y+84} -28 ${y+84} -57 ${y+72} Z" />
  </g>

  <!-- near side limbs -->
  ${xe(-17,"leg-far")}
  ${xe(15,"leg-near")}
  ${be(46,"arm-near",Ge(36,$+88)+He(36,$+80))}
  ...
  <g class="braid">
    <path class="hair" d="M -30 ${h+18}
      C -44 ${h+40} -46 ${h+74} -40 ${h+104}
      L -22 ${h+100}
      C -28 ${h+72} -26 ${h+44} -16 ${h+26} Z" />
    <circle class="ribbon" cx="-31" cy="${h+106}" r="6.5" />
    <path class="hair" d="M 32 ${h+18}
      C 46 ${h+40} 48 ${h+74} 42 ${h+104}
      L 24 ${h+100}
      C 30 ${h+72} 28 ${h+44} 18 ${h+26} Z" />
    <circle class="ribbon" cx="33" cy="${h+106}" r="6.5" />
  </g>
  ...
  <g class="head">
    <path class="hair" d="M -40 ${h+6}
      C -44 ${h-34} -20 ${h-54} 2 ${h-54}
      C 26 ${h-54} 46 ${h-32} 44 ${h+8}
      C 43 ${h+26} 40 ${h+36} 34 ${h+46}
      L -30 ${h+46}
      C -37 ${h+34} -40 ${h+22} -40 ${h+6} Z" />

    <!-- ears, with the gold drops the reference hangs from them -->
    <ellipse class="skin ear" cx="-37" cy="${h+6}" rx="7" ry="9" />
    <ellipse class="skin ear" cx="41" cy="${h+6}" rx="7" ry="9" />
    <circle class="earring" cx="-37" cy="${h+18}" r="3.6" />
    <circle class="earring" cx="41" cy="${h+18}" r="3.6" />

    <circle class="skin" cx="2" cy="${h}" r="${Ee}" />

    <!-- the parting: two sweeps meeting at the centre -->
    <path class="hair" d="M -38 ${h+2}
      C -41 ${h-26} -22 ${h-46} 2 ${h-46}
      C 0 ${h-30} -8 ${h-22} -22 ${h-14}
      C -31 ${h-8} -36 ${h-3} -38 ${h+2} Z" />
    <path class="hair" d="M 42 ${h+2}
      C 44 ${h-26} 26 ${h-46} 2 ${h-46}
      C 6 ${h-30} 16 ${h-22} 29 ${h-14}
      C 37 ${h-8} 40 ${h-3} 42 ${h+2} Z" />

    <!-- bindi -->
    <circle class="bindi" cx="2" cy="${h-20}" r="3.4" />

    <g class="face">
      <circle class="blush" cx="-20" cy="${h+13}" r="8" />
      <circle class="blush" cx="24" cy="${h+13}" r="8" />
      ...
      <g class="eyes">
        <g class="eye">
          <ellipse class="eye-white" cx="-12" cy="${h-2}" rx="9" ry="10.5" />
          <circle class="pupil" cx="-11" cy="${h-1}" r="6.4" />
          <circle class="glint" cx="-14" cy="${h-5}" r="2.6" />
          <circle class="glint small" cx="-8" cy="${h+3}" r="1.3" />
          <path class="lash" d="M -22 ${h-8} C -18 ${h-14} -6 ${h-14} -3 ${h-7}" />
        </g>
        <g class="eye">
          <ellipse class="eye-white" cx="16" cy="${h-2}" rx="9" ry="10.5" />
          <circle class="pupil" cx="17" cy="${h-1}" r="6.4" />
          <circle class="glint" cx="14" cy="${h-5}" r="2.6" />
          <circle class="glint small" cx="20" cy="${h+3}" r="1.3" />
          <path class="lash" d="M 7 ${h-7} C 10 ${h-14} 22 ${h-14} 26 ${h-8}" />
        </g>
      </g>

      <path class="nose" d="M 2 ${h+8} q 3.6 3 0 5.2" />

      <!-- every mood is drawn once; CSS reveals one -->
      <g class="brows">
        <g class="brow-set" data-for="neutral">
          <path d="M -22 ${h-17} Q -12 ${h-23} -3 ${h-18}" />
          <path d="M 7 ${h-18} Q 16 ${h-23} 26 ${h-17}" />
        </g>
        <g class="brow-set" data-for="curious">
          <path d="M -22 ${h-18} Q -12 ${h-26} -3 ${h-20}" />
          <path d="M 7 ${h-24} Q 16 ${h-32} 26 ${h-23}" />
        </g>
        <g class="brow-set" data-for="surprised">
          <path d="M -23 ${h-25} Q -12 ${h-34} -2 ${h-26}" />
          <path d="M 6 ${h-26} Q 16 ${h-34} 27 ${h-25}" />
        </g>
        <g class="brow-set" data-for="confused">
          <path d="M -22 ${h-22} Q -12 ${h-14} -3 ${h-20}" />
          <path d="M 7 ${h-26} Q 16 ${h-33} 26 ${h-24}" />
        </g>
        <g class="brow-set" data-for="happy">
          <path d="M -22 ${h-20} Q -12 ${h-27} -3 ${h-21}" />
          <path d="M 7 ${h-21} Q 16 ${h-27} 26 ${h-20}" />
        </g>
      </g>

      <g class="mouths">
        <path class="mouth-set" data-for="neutral"   d="M -6 ${h+21} Q 2 ${h+27} 10 ${h+21}" />
        <path class="mouth-set" data-for="curious"   d="M -5 ${h+20} Q 2 ${h+28} 9 ${h+20}" />
        <path class="mouth-set open" data-for="surprised" d="M 2 ${h+24} m -6 0 a 6 7 0 1 0 12 0 a 6 7 0 1 0 -12 0" />
        <path class="mouth-set" data-for="confused" d="M -6 ${h+25} Q 2 ${h+19} 10 ${h+24}" />
        <path class="mouth-set open" data-for="happy" d="M -8 ${h+19} Q 2 ${h+33} 12 ${h+19} Z" />
      </g>
    </g>
  </g>
  ...
  <g class="mithu-perch" transform="translate(44 ${$-10}) scale(0.28)">
    <g class="mithu-hop">${ze()}</g>
  </g>
...
<g class="mithu">
  ...
  <g class="mithu-feet">
    ...
    <circle cx="${e}" cy="${s-o-34}" r="7" />
          ...
          Q ${e} ${s} ${i} ${s-a*.28}
          Q ${e+t} ${s} ${e+t} ${s+a*.72}
          L ${e+t} ${s+o} Z`}function me(e,s,t,o,a="arch"){return`<path class="${a}" d="${le(e,s,t,o)}" />`}function ae(e,s,t,o,a=0){return`<path class="lit-window" style="animation-delay:${a}s" d="${le(e,s,t,o)}" />`}function K(e,s,t,o,a,i,l="arch"){const c=l==="lit"?ae:me;return Array.from({length:t},(d,u)=>l==="lit"?c(e+u*(o+i),s,o,a,u*.83%5):c(e+u*(o+i),s,o,a)).join("")}function ne(e,s,t,o=30){const a=o/2;return`<g class="stone">
  ...
  </g>${te(e,s-o,t*.9,o*.7)}`}function ge(e=0,s=18,t={x:0,y:0,w:1600,h:900}){let o=e*9301+49297;const a=()=>(o=(o*9301+49297)%233280)/233280;return`<g class="motes">${Array.from({length:s},()=>{const i=t.x+a()*t.w,l=t.y+a()*t.h,c=2+a()*2.6,d=(a()*9).toFixed(2),u=(7+a()*7).toFixed(2);return`<circle cx="${i}" cy="${l}" r="${c}" style="animation-delay:${d}s; animation-duration:${u}s" />`}).join("")}</g>`}function ve(e,s,t,o,a=2.6){const i=e+t/2;return`<path class="light-shaft" d="M ${e} ${s} L ${e+t} ${s}
    L ${i+t*a/2} ${s+o} L ${i-t*a/2} ${s+o} Z" />`}function Ue(){return`
<defs>
...
</defs>`}const M={floorY:420,taraX:60,wallX:-1090,doorX:390};function Be(){const{floorY:e}=M;return`
<g class="room">
  <!-- back wall and floor -->
...
</g>`}const A={x0:560,x1:3180,floorY:M.floorY,centreX:1840,askX:1760};function De(e,s){return`
<g class="fountain">
...
</g>`}function Ye(){const{x0:e,x1:s,floorY:t,centreX:o}=A,a=-760;return`
<g class="courtyard">
...
</g>`}const Y={x0:-1560,x1:3520,roofY:-1320,baseY:1010};function Je(){return`
<g class="kingdom">
  ...
  <path class="kd-hills far" d="M -6000 980 Q -3000 600 -600 940 Q 1800 1240 4400 860 Q 7400 520 10000 960
    L 10000 3400 L -6000 3400 Z" />
</g>`}function es(){const{x0:e,x1:s,roofY:t,baseY:o}=Y;return`
<g class="shell">
...
</g>`}function ss(){const e={x0:-1200,x1:540,y0:-640,y1:M.floorY+120},s={x0:A.x0-20,x1:A.x1+20,y0:-800,y1:A.floorY+200},t={x0:Y.x0-80,x1:Y.x1+80,y0:Y.roofY-160,y1:Y.baseY+90};return`
<g class="layer-glows">
  <!-- 1 · her room -->
  <g class="lg lg-local">
...
<g class="mlabel mlabel-${e}" transform="translate(${t} ${o})">
  <g class="ml-motes">
    ${[[-150,-30,0],[140,-44,.6],[-60,46,1.2],[110,40,1.8],[10,-62,.9]].map(([l,c,d])=>`<circle r="7" style="--mx:${l}px; --my:${c}px; animation-delay:${d}s" />`).join("")}
  </g>
...
<g class="trails">
...
</g>`}const n={x:1600,y:300,w:380,h:520,scale:.225},E=980,ke=(()=>{let e=12345;const s=()=>(e=(e*9301+49297)%233280)/233280;return Array.from({length:44},()=>({x:-1400+s()*5e3,y:-1500+s()*1900,r:1.6+s()*3.4,d:(s()*6).toFixed(2),layer:s()<.4?"far":"near"}))})();function se(e,s,t,o,a){return`<g class="cloud" style="animation-delay:${o}s; animation-duration:${a}s"
  ...
  </g>`}function he(e,s,t,o){return`<g class="flagpole-g">
...
<g class="sky-group">
  ...
  <g class="stars far">
    ${ke.filter(e=>e.layer==="far").map((e,s)=>`<circle class="${s%2?"still":""}" cx="${e.x}" cy="${e.y}" r="${e.r*.7}" style="animation-delay:${e.d}s" />`).join("")}
  </g>
  <g class="stars near">
    ${ke.filter(e=>e.layer==="near").map((e,s)=>`<circle class="${s%2?"still":""}" cx="${e.x}" cy="${e.y}" r="${e.r}" style="animation-delay:${e.d}s" />`).join("")}
  </g>
...
${ge(3,14,{x:700,y:200,w:1800,h:800})}`}function rs(){return`<clipPath id="heroClip">
...
</g>`}function Te(e){const s=Math.max(190,e.length*30+84);return`
...
<nav class="journey" aria-label="Story progress">
  ...
  <ol class="journey-steps">
    ...
    </div>`;const s=e.querySelector(".world"),t=new je(s,D),o=e.querySelector(".tara-slot"),a=ue(Xe());o.appendChild(a);const i=new Qe(a,{x:60,y:420,scale:1,facing:"left"}),l=ue(Se()+Te("laddoo"),"spark-slot");o.appendChild(l);const c=new Ae(l);c.at(520,-300);const d=ue(Se()+Te("chameli"),"spark-slot hers");o.appendChild(d);const u=new Ae(d);u.at(0,0).setScale(.06);const m=new Ie(e.querySelector(".code-air")),w=e.querySelector(".sr-live"),b=e.querySelector(".ui"),v=()=>{if(!b.classList.contains("show-line")){w.textContent="";return}const x=b.dataset.line,L=x?e.querySelector(`.narration[data-for="${x}"]`):e.querySelector('.narration[data-for="far"]'),H=L?L.textContent.trim():"";w.textContent!==H&&(w.textContent=H)};new MutationObserver(v).observe(b,{attributes:!0,attributeFilter:["class","data-line"]});const k=new qs(e.querySelector(".journey"));k.at("local");const S={root:e,camera:t,tara:i,spark:c,hers:u,code:m,journey:k,ui:e.querySelector(".ui")},f=()=>{Ze(),e.querySelector(".hero-window").classList.remove("is-open","is-passed"),e.querySelector(".portal").classList.remove("is-inside"),l.classList.remove("is-live","is-word","is-dimmed"),d.classList.remove("is-live","is-word","is-bouncing","is-straining","is-fading"),u.at(0,0).setScale(.06),e.querySelector(".room").classList.remove("wall-felt"),e.querySelector(".room-door").classList.remove("is-noticed"),e.querySelector(".tara").classList.remove("mithu-alert"),e.querySelector(".wall-ripple").classList.remove("is-hit"),e.querySelector(".room-door").classList.remove("is-open"),e.querySelector(".portal").classList.remove("show-shell"),e.querySelector(".layer-glows").classList.remove("is-live"),e.querySelectorAll(".lg").forEach(L=>L.classList.remove("is-lit")),e.querySelectorAll(".mlabel").forEach(L=>L.classList.remove("is-named")),e.querySelector(".ask-prompt").classList.remove("is-offered","is-taken"),e.querySelector(".ask-speech").classList.remove("is-spoken"),e.querySelector(".q-motes").classList.remove("is-asking"),e.querySelector(".courtyard").classList.remove("is-hushed"),d.classList.remove("is-homebound","is-beckoning","is-inner","is-shadowing","is-touched"),l.classList.remove("is-outer","is-answering","is-shadowed","is-touched"),u.setText("chameli"),c.setText("laddoo"),e.querySelector(".reach").classList.remove("show-global","show-nonlocal","show-shadow"),e.querySelectorAll(".lg").forEach(L=>L.classList.remove("is-probing","is-empty","is-found")),e.querySelectorAll(".mlabel").forEach(L=>L.classList.remove("is-inline")),e.querySelector(".layer-glows").classList.remove("is-searching"),e.querySelector(".tara").classList.remove("has-lantern","has-megaphone","mithu-away"),d.classList.remove("is-hollow","is-claimed","is-hollow-pulse","is-clearing"),e.querySelector(".trails").classList.remove("show-out","show-local","show-blocked","is-barred"),e.querySelector(".error-spell").classList.remove("is-cast"),e.querySelector(".perch-stand").classList.remove("is-up"),e.querySelector(".shadow-tag .mlabel").classList.remove("is-named"),e.querySelector(".stage").classList.remove("is-darkened"),m.clear({duration:0}),m.undock(),e.querySelector(".tara").classList.remove("looks-out","mithu-nods"),e.querySelector(".portal").classList.remove("is-warming"),e.querySelectorAll(".is-waking").forEach(L=>L.classList.remove("is-waking")),e.querySelectorAll(".is-fading").forEach(L=>L.classList.remove("is-fading")),d.classList.remove("is-settled","is-rising","is-star"),l.classList.remove("is-settled"),e.querySelector(".layer-glows").classList.remove("is-soft"),e.querySelector(".replay").classList.remove("is-offered"),e.querySelector(".interact").innerHTML="",k.reset(),k.at("local"),Pe(),e.querySelector(".stage").classList.remove("is-paused");const x=e.querySelector(".pausebtn");x.classList.remove("is-paused"),x.setAttribute("aria-pressed","false"),x.setAttribute("aria-label","Pause the story"),S.ui.classList.remove("is-ending"),delete S.ui.dataset.line,S.ui.classList.remove("show-title","show-sub","show-line"),i.at(60,420),i.facing="left",i.apply(),c.x=520,c.y=-300,c.apply(),t.unfollow(),Ss(S)},C=e.querySelector(".pausebtn"),I=e.querySelector(".stage"),G=x=>{I.classList.toggle("is-paused",x),C.classList.toggle("is-paused",x),C.setAttribute("aria-pressed",String(x)),C.setAttribute("aria-label",x?"Resume the story":"Pause the story")};return C.addEventListener("click",()=>G($e())),addEventListener("keydown",x=>{if(x.code!=="Space"&&x.key!==" ")return;const L=x.target;L instanceof Element&&(L.closest("button")||L.getAttribute("role")==="button"||L.isContentEditable)||(x.preventDefault(),G($e()))}),e.querySelector(".replay").addEventListener("click",f),F?(e.querySelector(".hero-window").classList.add("is-open","is-passed"),e.querySelector(".portal").classList.add("is-inside","show-shell"),e.querySelector(".room-door").classList.add("is-open"),d.classList.add("is-live","is-word","is-homebound"),u.at(-620,-240).setScale(1),l.classList.add("is-live","is-word"),c.at(470,-330),i.at(1760,420),i.express("confused"),e.querySelector(".layer-glows").classList.add("is-live"),e.querySelectorAll(".lg").forEach(x=>x.classList.add("is-lit")),e.querySelectorAll(".mlabel").forEach(x=>x.classList.add("is-named")),u.setText("TARA"),d.classList.add("is-inner"),u.at(-560,-300),c.setText("MITHU"),l.classList.add("is-outer"),c.at(1900,-1180),t.set({x:n.x+980*n.scale,y:n.y+-940*n.scale,zoom:t.zoomToFitWidth(1760)}),m.dock(),m.write(['name = "Mithu"',"","def room():",'    name = "Tara"',"    print(name)"],{stagger:0}),m.note(0,"Global — the palace name"),m.note(3,"assigning makes a NEW local name"),m.note(4,"finds the local one first"),S.ui.dataset.line="names",S.ui.classList.add("show-line")):f(),S}Ms(document.getElementById("app"));
